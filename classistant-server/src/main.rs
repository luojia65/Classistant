    ...
    thread::spawn(move || {
        ...
        HttpServer::new(move || {
            App::new()
                ...
                // .route("/v1/users/{}", web::delete().to(auth::unregister))
                // .route("/v1/sessions", web::post().to(auth::login))
                // .route("/v1/sessions/{}", web::delete().to(auth::logout))
                // .route("/v1/users/{}/data", web::get().to(data_user::get))
                // .route("/v1/users/{}/data", web::post().to(data_user::create))
